...
def try_store_swap():
    global store_uses_left, cards, store_selected_indices, store_target_type, gate_message, gate_message_timer
    ...
    chosen = [cards[i] for i in store_selected_indices]

    # same type + same power
    if chosen[0]["type"] != chosen[1]["type"]:
        gate_message = "CARDS MUST MATCH TYPE!"
        gate_message_timer = 90
        return False

    if chosen[0]["power"] != chosen[1]["power"]:
        gate_message = "CARDS MUST MATCH POWER!"
        gate_message_timer = 90
        return False

    if store_target_type is None or store_target_type == chosen[0]["type"]:
        gate_message = "CHOOSE DIFFERENT TYPE!"
        gate_message_timer = 90
        return False

    power = chosen[0]["power"]
    ...
    gate_message = "STORE TRADE SUCCESS!"
    gate_message_timer = 90
    return True
...
def draw_store_ui():
    ...
    screen.blit(title, (SIDEBAR_W//2 - title.get_width()//2, base_y))

    uses = retro_small.render(f"USES LEFT: {store_uses_left}", True, (200,200,200))
    screen.blit(uses, (20, base_y + 40))

    hint = retro_small.render("GIVE 2 SAME → GET 1", True, (180,180,180))
    screen.blit(hint, (20, base_y + 60))
    ...
    for i, t in enumerate(CARD_TYPES):
        ...
        screen.blit(txt, (20 + i*80, base_y + 90))

    # button
    btn = pygame.Rect(20, base_y + 130, 160, 36)
...
while True:
    ...
    for e in pygame.event.get():
        ...
        if e.type == pygame.KEYDOWN and e.key == pygame.K_e:
            pressed_e = True
        if e.type == pygame.MOUSEBUTTONDOWN and show_swap_ui:
            mx, my = e.pos
        ...
        # STORE CARD SELECTION (RIGHT CLICK)
        if e.button == 3:  # right-click
            for i, c in enumerate(cards):
                ...
                base_y = SCREEN_HEIGHT - 220
                for i, t in enumerate(CARD_TYPES):
                    r = pygame.Rect(20 + i*80, base_y + 90, 70, 20)
