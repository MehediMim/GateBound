...
def new_room():
    ...
    rooms[rid] = {
        ...
        "links": {},
        "map_pos": None   # (x, y) on minimap grid
    }
    rid += 1
    return rid - 1
...
def connect_rooms(a, direction, b):
    ...
    ax, ay = rooms[a]["map_pos"]

    if direction == "top":
        rooms[b]["map_pos"] = (ax, ay - 1)
    elif direction == "bottom":
        rooms[b]["map_pos"] = (ax, ay + 1)
    elif direction == "left":
        rooms[b]["map_pos"] = (ax - 1, ay)
...
def ensure_neighbors(room_id):
    ...
    # Force bottom creation (artificial)
    if "bottom" not in room["links"]:
        parent = new_room()
        connect_rooms(room_id, "bottom", parent)

# Start game
current = new_room()
rooms[current]["map_pos"] = (0, 0)
...
def draw_debug_info():
    ...
    def draw_minimap():
    # Panel background
    panel_x = SCREEN - MAP_SIZE - MAP_PADDING
    panel_y = MAP_PADDING
    pygame.draw.rect(
        screen, (20, 20, 20),
        (panel_x, panel_y, MAP_SIZE, MAP_SIZE)
    )
    pygame.draw.rect(
        screen, (200, 200, 200),
        (panel_x, panel_y, MAP_SIZE, MAP_SIZE), 2
    )

    # Find map bounds
    xs = [room["map_pos"][0] for room in rooms.values() if room["map_pos"]]
    ys = [room["map_pos"][1] for room in rooms.values() if room["map_pos"]]
    ...
    scale = min(
        MAP_SIZE / (span_x * MAP_NODE_SIZE),
        MAP_SIZE / (span_y * MAP_NODE_SIZE),
        1.0
    )

    # Draw connections first
    for room in rooms.values():
        if room["map_pos"] is None:
            continue

        x, y = room["map_pos"]
        sx = panel_x + (x - min_x) * MAP_NODE_SIZE * scale + MAP_NODE_SIZE
        sy = panel_y + (y - min_y) * MAP_NODE_SIZE * scale + MAP_NODE_SIZE

        for nb in room["links"].values():
            nb_room = rooms[nb]
            nx, ny = nb_room["map_pos"]
            ex = panel_x + (nx - min_x) * MAP_NODE_SIZE * scale + MAP_NODE_SIZE
            ey = panel_y + (ny - min_y) * MAP_NODE_SIZE * scale + MAP_NODE_SIZE
            pygame.draw.line(screen, (120, 120, 120), (sx, sy), (ex, ey), 2)
    ...
    for rid, room in rooms.items():
        ...
        x, y = room["map_pos"]
        rx = panel_x + (x - min_x) * MAP_NODE_SIZE * scale
        ry = panel_y + (y - min_y) * MAP_NODE_SIZE * scale

        color = (0, 200, 255) if rid == current else (180, 180, 180)

        pygame.draw.rect(
            screen, color,
            (rx, ry, MAP_NODE_SIZE * scale, MAP_NODE_SIZE * scale)
        )

        txt = font.render(str(rid), True, (0, 0, 0))
        screen.blit(
            txt,
            (rx + 3, ry + 2)
        )
