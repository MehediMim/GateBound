...
GATES = {
    ...
    "bottom": pygame.Rect(
        ROOM_RECT.centerx - 56,
        ROOM_RECT.bottom - 28 - GATE_THICK,
        112,
        GATE_THICK
    ),
    "left": pygame.Rect(
        ROOM_RECT.left + 28,
        ROOM_RECT.centery - 56,
        GATE_THICK,
        112
    ),
    "right": pygame.Rect(
        ROOM_RECT.right - 28 - GATE_THICK,
        ROOM_RECT.centery - 56,
        GATE_THICK,
        112
    ),
...
def move(dx, dy):
    ...
    allow_gate = keys[pygame.K_e]

    # ---- X movement ----
    player.x += dx

    for w in get_blocking_walls():
        # allow passing ONLY if colliding with gate + E
        if allow_gate and any(player.colliderect(g) for g in GATES.values()):
            continue
...
def draw_press_e_hint():
    for r in DOORS.values():
        if player.colliderect(r):
            ...
            screen.blit(txt, (player.x - 10, player.y - 20))
...
def draw_debug_borders():
    ...
    for g in GATES.values():
        pygame.draw.rect(screen, (0, 255, 0), g, 2)
