...
while True:
    ...
    # Draw neighbors first (behind), then center on top
    # Bottom (previous)
    if room["from"] is not None:
        prev_room = rooms[room["from"]]
        blit_half(BG[prev_room["type"]], 0, +HALF)

    # Top/Left/Right neighbors (saved for this room)
    blit_half(BG[rooms[room["next"]["top"]]["type"]],   0, -HALF)
    blit_half(BG[rooms[room["next"]["left"]]["type"]], -HALF, 0)
    blit_half(BG[rooms[room["next"]["right"]]["type"]], +HALF, 0)

    # Center room last (on top)
    blit_center(BG[room["type"]])
