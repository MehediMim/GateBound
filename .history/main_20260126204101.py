...
def try_swap_with_gate(d):
    global cards

    gate_card = gate_cards[d]
    required_type = get_next_room_type(d)

    for i, c in enumerate(cards):
        if c["type"] == required_type and c["power"] >= gate_card["power"]:
            # swap
            cards[i], gate_cards[d] = gate_cards[d], cards[i]
            gate_unlocked[d] = True
            return True
...
def draw_gate_card_popup():
    ...
    x = ROOM_RECT.centerx - CARD_WIDTH // 2
    y = ROOM_RECT.centery - CARD_HEIGHT // 2

    draw_full_card(card, x, y)

    hint = retro_small.render("Press E to swap", True, (255,255,255))
    screen.blit(hint, (x + CARD_WIDTH//2 - hint.get_width()//2, y - 20))
    
def draw_swap_button():
    if not show_swap_ui or selected_card_index is None:
        return None
...
while True:
    ...
    if show_swap_ui and e.type == pygame.MOUSEBUTTONDOWN:
        ...
        if btn and btn.collidepoint(e.pos):
            d = can_interact_gate()
            if d is not None and selected_card_index is not None:
                if try_swap_with_gate(d):
                    selected_card_index = None
