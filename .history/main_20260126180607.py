...
def draw_minimap():
    ...
    cx = panel_x + panel_size // 2
    cy = panel_y + panel_size // 2
    ...
    center_room = rooms[current]

    # Draw center
    pygame.draw.rect(
        screen, (0, 200, 255),
        (*positions["center"], node, node), border_radius=3
    )
    screen.blit(map_font.render(str(current), True, (0, 0, 0)),
                (positions["center"][0] + 5, positions["center"][1] + 4))

    # Draw neighbors
    for d in ("top", "left", "right", "bottom"):
        nb = center_room["links"].get(d)
        if nb is None:
            continue
        ...
        pygame.draw.line(screen, (120, 120, 120),
                         (cx + node // 2, cy + node // 2),
                         (px + node // 2, py + node // 2), 2)

        pygame.draw.rect(
            screen, (180, 180, 180),
            (px, py, node, node), border_radius=3
        )

        screen.blit(map_font.render(str(nb), True, (0, 0, 0)),
                    (px + 5, py + 4))
