...
def can_use_card_for_gate(card, d):
    ...
    # type must match next room
    if card["type"] != get_next_room_type(d):
        return False

    # power must be <= required power
    if card["power"] > gate_cards[d]["power"]:
        return False
...
def handle_doors():
    ...
    for d, r in DOORS.items():
        if not gate_unlocked[d]:
            continue
        if player.colliderect(r):
            current = rooms[current]["links"].get(d, current)
            player.center = SPAWN

            # reset gate states for new room
            for k in gate_unlocked:
                gate_unlocked[k] = False
