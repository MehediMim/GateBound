...
def draw_cards_title():
    ...
    y = SIDEBAR_HUD_Y + SIDEBAR_HUD_H + 10
...
def draw_sidebar_hud():
    ...
    shadow = pygame.Surface((w, HUD_H), pygame.SRCALPHA)
    shadow.fill((0,0,0,80))
    screen.blit(shadow, (x+3, y+3))
    screen.blit(hud_bg, (x, y))

    cx = x + w // 2
    y0 = y + 26
    gap = 26

    draw_hud_line(f"POINTS: {points}", cx, y0, HUD_FONT_BIG)
    draw_hud_line(f"START ROOM: #{START_ROOM}", cx, y0+gap, HUD_FONT_NORMAL)
    draw_hud_line(f"END ROOM: #{finish_room}", cx, y0+gap*2, HUD_FONT_NORMAL, (255,80,80))
    draw_hud_line(
        f"{rooms[current]['type'].upper()} CHAMBER  -  #{current}",
        cx,
        y0+gap*3,
